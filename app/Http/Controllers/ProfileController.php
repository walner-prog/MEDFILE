...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('home', $user)->with('success', 'El perfil se actualizó con éxito');
        } catch (\Exception $e) {
            // Revertir la transacción si algo falla
            DB::rollBack();

            // Manejar el error (puedes personalizar el mensaje de error según sea necesario)
            return redirect()->route('profile.edit')->withErrors(['error' => 'Ocurrió un error al actualizar el perfil. Por favor, intenta nuevamente.']);
        }
    }
}
